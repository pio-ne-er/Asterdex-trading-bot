...
const exchangePrivate = new ccxt.binance({
  apiKey: process.env.EXCHANGE_A_API_KEY,
  secret: process.env.EXCHANGE_A_API_SECRET,
  options: {
    defaultType: "swap",
  },
  urls: {
...
async function orderStatusWatcher() {
  while (true) {
    ...
    for (let i = pendingOrders.length - 1; i >= 0; i--) {
      ...
      try {
        const order = await exchangePrivate.fapiPrivateGetOrder({ symbol: TRADE_SYMBOL, orderId });
        if (order) {
...
async function placeOrder(side: "BUY" | "SELL", price: number, amount: number, reduceOnly = false): Promise<any> {
  try {
    ...
    const order = await exchangePrivate.fapiPrivatePostOrder(params);
...
async function getPosition() {
  try {
    const account = await exchangePrivate.fapiPrivateV2GetAccount();
...
async function marketClose(side: "SELL" | "BUY") {
  try {
    await exchangePrivate.fapiPrivatePostOrder({
      symbol: TRADE_SYMBOL,
      side,
      type: "MARKET",
...
async function ensureNoPendingReduceOnly(side: "BUY" | "SELL", price: number) {
  ...
  const openOrders = await exchangePrivate.fapiPrivateGetOpenOrders({ symbol: TRADE_SYMBOL });
  return !openOrders.some((o: any) => o.side === side && o.reduceOnly && parseFloat(o.price) === price);
}

async function cancelAllOrders() {
  try {
    await exchangePrivate.fapiPrivateDeleteAllOpenOrders({ symbol: TRADE_SYMBOL });
  } catch (e) {
    ...
    try {
      ...
      if (pos.positionAmt > -0.00001 && pos.positionAmt < 0.00001) {
        ...
        while (!filled) {
          ...
          // 检查订单状态
          const buyOrderStatus = orderBuy ? await exchangePrivate.fapiPrivateGetOrder({ symbol: TRADE_SYMBOL, orderId: orderBuy.orderId }) : null;
          const sellOrderStatus = orderSell ? await exchangePrivate.fapiPrivateGetOrder({ symbol: TRADE_SYMBOL, orderId: orderSell.orderId }) : null;
        ...
        for (const o of openOrders) {
          if (o.side !== closeSide || o.reduceOnly !== true || parseFloat(o.price) !== closePrice) {
            await exchangePrivate.fapiPrivateDeleteOrder({ symbol: TRADE_SYMBOL, orderId: o.orderId });
            console.log(`[撤销非平仓方向挂单] 订单ID: ${o.orderId} side: ${o.side} price: ${o.price}`);
          }
        }
        // 检查是否已挂平仓方向的单
        const stillOpenOrders = await exchangePrivate.fapiPrivateGetOpenOrders({ symbol: TRADE_SYMBOL });
