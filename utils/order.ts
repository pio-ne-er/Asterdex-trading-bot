import { Aster, CreateOrderParams, AsterOrder } from "../exchanges/exchange-adapter";
...
export async function deduplicateOrders(exchange: Aster, openOrders: AsterOrder[], orderTypeLocks: { [key: string]: boolean }, orderTypeUnlockTimer: { [key: string]: NodeJS.Timeout | null }, orderTypePendingOrderId: { [key: string]: string | null }, type: string, side: string, logTrade: (type: string, detail: string) => void) {
  ...
  if (orderIdList.length > 0) {
    try {
      ...
      await exchange.cancelOrders({ symbol: TRADE_SYMBOL, orderIdList });
...
export async function placeOrder(exchange: Aster, openOrders: AsterOrder[], orderTypeLocks: { [key: string]: boolean }, orderTypeUnlockTimer: { [key: string]: NodeJS.Timeout | null }, orderTypePendingOrderId: { [key: string]: string | null }, side: "BUY" | "SELL", price: number, amount: number, logTrade: (type: string, detail: string) => void, reduceOnly = false) {
  const type = "LIMIT";
...
export async function placeStopLossOrder(exchange: Aster, openOrders: AsterOrder[], orderTypeLocks: { [key: string]: boolean }, orderTypeUnlockTimer: { [key: string]: NodeJS.Timeout | null }, orderTypePendingOrderId: { [key: string]: string | null }, tickerSnapshot: any, side: "BUY" | "SELL", stopPrice: number, logTrade: (type: string, detail: string) => void) {
...
export async function marketClose(exchange: Aster, openOrders: AsterOrder[], orderTypeLocks: { [key: string]: boolean }, orderTypeUnlockTimer: { [key: string]: NodeJS.Timeout | null }, orderTypePendingOrderId: { [key: string]: string | null }, side: "SELL" | "BUY", logTrade: (type: string, detail: string) => void) {
  const type = "MARKET";
  if (isOperating(orderTypeLocks, type)) return;
  const params: CreateOrderParams = {
    symbol: TRADE_SYMBOL,
...
export async function placeTrailingStopOrder(exchange: Aster, openOrders: AsterOrder[], orderTypeLocks: { [key: string]: boolean }, orderTypeUnlockTimer: { [key: string]: NodeJS.Timeout | null }, orderTypePendingOrderId: { [key: string]: string | null }, side: "BUY" | "SELL", activationPrice: number, quantity: number, logTrade: (type: string, detail: string) => void) {
