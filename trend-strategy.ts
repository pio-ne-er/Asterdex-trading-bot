...
} from "./exchanges/exchange-adapter";
...
const exchange = new Aster(
  process.env.EXCHANGE_A_API_KEY!,
  process.env.EXCHANGE_A_API_SECRET!
);
...
// 订阅所有推送
exchange.watchAccount((data) => {
  accountSnapshot = data;
  // 账户更新不再直接解锁
});
exchange.watchOrder((orders: AsterOrder[]) => {
...
exchange.watchDepth(TRADE_SYMBOL, (depth: AsterDepth) => {
  depthSnapshot = depth;
});
exchange.watchTicker(TRADE_SYMBOL, (ticker: AsterTicker) => {
  tickerSnapshot = ticker;
});
exchange.watchKline(TRADE_SYMBOL, "1m", (klines: AsterKline[]) => {
...
async function handleOpenPosition(lastPrice: number | null, lastSMA30: number, price: number, openOrders: AsterOrder[], orderTypeLocks: any, exchange: Aster, TRADE_SYMBOL: string, TRADE_AMOUNT: number, tradeLog: TradeLogItem[], logTrade: any, lastOpenOrder: any) {
  // 撤销所有普通挂单和止损单
  if (openOrders.length > 0) {
    isOperating(orderTypeLocks, "MARKET");
    await exchange.cancelAllOrders({ symbol: TRADE_SYMBOL });
  }
  // 仅在价格穿越SMA30时下市价单
  if (lastPrice !== null) {
...
async function handlePositionManagement(pos: any, price: number, lastSMA30: number, openOrders: AsterOrder[], orderTypeLocks: any, orderTypeUnlockTimer: any, orderTypePendingOrderId: any, tickerSnapshot: any, exchange: Aster, TRADE_SYMBOL: string, LOSS_LIMIT: number, TRAILING_PROFIT: number, tradeLog: TradeLogItem[], logTrade: any, lastCloseOrder: any, lastStopOrder: any, marketClose: any, placeStopLossOrder: any, placeTrailingStopOrder: any, toPrice1Decimal: any) {
  ...
  if (pnl > 0.1 || pos.unrealizedProfit > 0.1) {
    if (!currentStopOrder) {
      ...
      if (Math.abs(curStopPrice - profitMoveStopPrice) > 0.01) {
        isOperating(orderTypeLocks, "MARKET");
        await exchange.cancelOrder({ symbol: TRADE_SYMBOL, orderId: currentStopOrder.orderId });
        isOperating(orderTypeLocks, "MARKET");
        await placeStopLossOrder(aster, openOrders, orderTypeLocks, orderTypeUnlockTimer, orderTypePendingOrderId, tickerSnapshot, stopSide, profitMoveStopPrice, (type: any, detail: any) => logTrade(tradeLog, type, detail));
...
async function trendStrategy() {
  ...
  while (true) {
    ...
    let pnl = 0;
    if (isNoPosition(pos)) {
      await handleOpenPosition(lastPrice, lastSMA30, price, openOrders, orderTypeLocks, exchange, TRADE_SYMBOL, TRADE_AMOUNT, tradeLog, logTrade, lastOpenOrder);
      lastStopOrder.side = null;
      lastStopOrder.price = null;
    } else {
      const result = await handlePositionManagement(pos, price, lastSMA30, openOrders, orderTypeLocks, orderTypeUnlockTimer, orderTypePendingOrderId, ticker!, exchange, TRADE_SYMBOL, LOSS_LIMIT, TRAILING_PROFIT, tradeLog, logTrade, lastCloseOrder, lastStopOrder, marketClose, placeStopLossOrder, placeTrailingStopOrder, toPrice1Decimal);
      pnl = result.pnl;
      if (result.closed) {
        totalTrades++;
        totalProfit += pnl;
